document.addEventListener('DOMContentLoaded', () => {
    ...
    btnMostrar.addEventListener('click', async () => {
        const compId = selectCompeticion.value;
        const selectedOpt = selectCompeticion.options[selectCompeticion.selectedIndex];
        const tipo = selectedOpt.dataset.tipo;
        
        const esLiga = (tipo === 'liga' || tipo === 'internacional_europa' || tipo === 'internacional_america');
        const urlBase = esLiga ? '/clasificacion' : '/copa';
        
        contenedor.innerHTML = '<div class="text-center mt-5"><div class="spinner-border text-primary"></div><p>Cargando datos...</p></div>';

        try {
            const res = await fetch(`${urlBase}/${partidaId}/${compId}?ajax=true`);
            if (!res.ok) throw new Error('Error en la respuesta');
            const html = await res.text();
            contenedor.innerHTML = html;
        } catch (err) {
            console.error(err);
            contenedor.innerHTML = '<div class="alert alert-danger">No se pudo cargar la información de esta competición.</div>';
        }
    });
});